...
const PrimaryButton = ({ children, className, onClick, onMouseDown, disabled }: { children: React.ReactNode, className?: string, onClick?: () => void, onMouseDown?: (e: React.MouseEvent) => void, disabled?: boolean }) => {
    return (
        <Button className="min-w-0 bg-gradient-to-t from-[#222222] to-[#303030] p-[3px] rounded-xl transition-opacity duration-300 cursor-pointer gb-blur-background-image" onClick={onClick} onMouseDown={onMouseDown} disabled={disabled}>
